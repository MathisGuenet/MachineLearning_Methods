...
```{r}
par(mfrow=c(1,1))
stripchart(data, pch=21, bg=data[,2])
```
...
# Q12 Apply a density estimate on your generated data and visualize it
```{r}
density3 = densityMclust(data[,1])
...
```{r}
plot(gmm3, what = 'BIC')
#we see that that k = 3 is the best :)
```

# Bonus uncertainty
```{r}
plot(gmm3, what = 'uncertainty')
#the uncertainty is higher for the 2 last gaussian. We can explain it because their distance to the mean is closer and std is alsmost the same and more higher than the first gaussian
```

# Q8.5 EM from scratch
#2.1 generate 2 dimensional dataset
```{r}
set.seed(123)
g1 <- rnorm(100, mean = 0, sd=3)
g2 <- rnorm(100, mean = 20, sd=5)
...
truth = c(rep(1,100),rep(2,100),rep(3,100))
#join the data
data = cbind(gauss1,gauss2,truth)
```

#2.2 Implementation of the EM algorithm to fit a GMM on your generated data
```{r}
